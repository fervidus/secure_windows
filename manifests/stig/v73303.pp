    ...
    $facts['windows_role'] =~ /(^184|,184,|,184$)/) {
      $ftp_sites.each |String $site| {
        ...
        exec { "Set FTP anynymousAuthentication to Disabled on ${site}":
          command => "${facts['os']['windows']['system32']}\\inetsrv\\AppCmd.exe set config -section:system.applicationHost/sites /[name='${site}'].ftpServer.security.authentication.anonymousAuthentication.enabled:\"False\" /commit:apphost", # lint:ignore:140chars
        }
      }
    }
  }
